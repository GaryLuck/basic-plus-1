...
// caused the panic.  Three cases here: implicit calls to panic by the
// Go runtime code, a call to fatalError, or a call to runtimeError.
...
func decodePanic(e any) {
	...
	case *crawloutException:
		...
		// We also need to nuke the current execution state to avoid
		// an attempt to continue if requested
		//
...
func floatValuesApproxEqual(f1, f2 float64) bool {
	...
	if basicFormat(f1) == basicFormat(f2) {
		return true
	} else {
		return false
	}
...
func runtimeErrorInternal(msg string, state *procState, err int16) {
	...
	// but did not return, then caused another exception
	//
...
//
// Take a custom runtimeErrorInfo structure, and gin up a standard
// Error string from it
//

func (re *runtimeErrorInfo) Error() string {
	return re.msg
}
...
func makeStmtNode(token int, operands ...*tokenNode) *stmtNode {
	...
	return &stmtNode{token: token, line: g.yylex.line, operands: operands}
}
...
func makeTokenNode(token int, operands ...any) *tokenNode {
	...
	// to do.  If 1, it's the left token node or a primitive data type.
...
func requireCompatibleOperands(onode int, lnode, rnode *tokenNode,
	...
	//
	// The left and right operands must both be string or numeric.
...
func expressionType(tnode *tokenNode) int {
	...
	case 0, 1:
		//	suff := "s"
		//	if len(tnode.operands) == 1 {
		//		suff = ""
		//	}
		//	fmt.Printf("\t%d operand%s - returning FLOAT\n",
		//		len(tnode.operands), suff)
		return FLOAT
...
func fetchArgList(tn *tokenNode, numArgs int) []*tokenNode {

	argList := make([]*tokenNode, 0)
	for np := tn; np != nil; np = np.next {
		argList = append(argList, np)
	}

	basicAssert(len(argList) == numArgs, "Expected %d args got %d",
		numArgs, len(argList))
...
//
// Check to ensure we are not entering or exiting a multi line
...
//
// This function takes a stmtNode and return a string or numeric
// representation of the statement number.  This handles the fact that
// if we have multiple statements on a line, only the first one has a
// non-zero stmtNo field.  Subsequent nodes point back to the head,
// so we look there
